...
function JsonViewer({ data, initialExpanded = false }: JsonViewerProps) {
            ...
            <>
              {Object.entries(value).map(([k, v], index) => (
                <div key={k} className="ml-4">
                  ...
                  {index < Object.keys(value).length - 1 && <span className="text-gray-500">,</span>}
                </div>
              ))}
              <div className="ml-4">
                <span className="text-gray-500">{isArray ? "]" : "}"}</span>
  ...
  return <pre className="bg-gray-100 p-4 rounded-lg overflow-auto text-sm">{renderValue(data, "root")}</pre>;
}
